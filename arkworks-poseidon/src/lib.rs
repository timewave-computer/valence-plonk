...
impl ConstraintSynthesizer<Fq> for PoseidonDemoCircuit {
    fn generate_constraints(self, cs: ConstraintSystemRef<Fq>) -> Result<(), SynthesisError> {
        ...
        let input_var = FpVar::new_witness(cs.clone(), || self.input.ok_or(SynthesisError::AssignmentMissing))?;

        // Allocate the Poseidon parameters as constants
        let params_var = <CRHGadget<Fq> as CRHSchemeGadget<CRH<Fq>, Fq>>::ParametersVar::new_constant(cs.clone(), &self.params)?;
        ...
        let expected_output_var = FpVar::new_input(cs, || self.expected_output.ok_or(SynthesisError::AssignmentMissing))?;
...
mod tests {
    ...
    use ark_groth16::{Groth16, r1cs_to_qap::LibsnarkReduction, Proof, VerifyingKey};
    use ark_snark::SNARK;
    ...
    fn test_poseidon_hash_proof() {
        ...
        let (pk, vk) = Groth16::<ark_bls12_381::Bls12_381, LibsnarkReduction>::circuit_specific_setup(setup_circuit, rng).unwrap();
        ...
        // Generate the proof
        let proof = Groth16::<ark_bls12_381::Bls12_381, LibsnarkReduction>::prove(&pk, proof_circuit, rng).unwrap();

        // The public input is the hash value
        let public_inputs = vec![final_hash];
        println!("Public input (hash after 100 iterations): {:?}", public_inputs[0]);

        // Verify the proof
        let is_valid = Groth16::<ark_bls12_381::Bls12_381, LibsnarkReduction>::verify(&vk, &public_inputs, &proof).unwrap();
        assert!(is_valid);
